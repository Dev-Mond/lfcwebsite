...
class modOsTouchSliderHelper{
  ...
  public static function getAjax() {
    ...
    if($task == 'save_settings'){
      ...
      if(isset($frontendParams['is_width_in_pixels']) && $frontendParams['is_width_in_pixels']){
        $params->set("is_width_in_pixels", $frontendParams['is_width_in_pixels']);
        $params->set("width_px", isset($frontendParams['width_px'])? $frontendParams['width_px'] : 400);
      }else{
        $params->set("is_width_in_pixels", (isset($frontendParams['is_width_in_pixels']))?($frontendParams['is_width_in_pixels']):("") ) ;
        $params->set("width_per", isset($frontendParams['width_per'])? $frontendParams['width_per'] : 100);
      }
      if(isset($frontendParams['is_height_in_pixels']) && $frontendParams['is_height_in_pixels']){
        $params->set("is_height_in_pixels", $frontendParams['is_height_in_pixels']);
        $params->set("height_px", isset($frontendParams['height_px'])? $frontendParams['height_px'] : 200);
      }else{
        $params->set("is_height_in_pixels", (isset($frontendParams['is_height_in_pixels']))?($frontendParams['is_height_in_pixels']):("") );
      ...
      $params->set("initialSlide", (isset($frontendParams['initialSlide']) && $frontendParams['initialSlide'] != 'NaN')? $frontendParams['initialSlide'] : 0);
      ...
      $pagination = isset($frontendParams['pagination'])? $frontendParams['pagination'] : '';
      $params->set("pagination", isset($pagination['el'])? $pagination['el'] : '.swiper-pagination');
      $params->set("paginationType", isset($pagination['type'])? $pagination['type'] : "bullets");
      $params->set("paginationClickable", isset($pagination['clickable'])? $pagination['clickable'] : 1);
      ...
      if(isset($frontendParams['loop']) && $frontendParams['loop'] == 1 ){
        //fix error with slider and loop, ovveride
        $params->set("slidesPerView", 1);
        $params->set("slidesPerColumn", 1);
        $params->set("spaceBetween", 0);
      } else {
      ...
      $params->set("lazy", isset($frontendParams['lazy'])? $frontendParams['lazy'] : 0);
      $params->set("loadPrevNext", isset($frontendParams['loadPrevNext'])? $frontendParams['loadPrevNext'] : 0);
      $params->set("loadPrevNextAmount", isset($frontendParams['loadPrevNextAmount'])? $frontendParams['loadPrevNextAmount'] : 1);
      ...
      $params->set("imageLink", isset($frontendParams['imageLink']) ? json_decode($frontendParams['imageLink']) : array());
      ...
      if(isset($textArray) && count($textArray)){
        ...
        foreach ($textArray as $imgId => $textHtmlAray){
          ...
          if($imgId && is_array($textHtmlAray) && 0 < count($textHtmlAray)){
    ...
    //end
      ...
      echo json_encode($response);
        ...
        ( $_FILES['qqslfile']['type'] == 'application/zip' ||
          $_FILES['qqslfile']['type'] == 'application/x-zip-compressed'
        ) ){
      ...
      $mime = mime_content_type($_FILES['qqslfile']['tmp_name']);
      $max_filesize = self::toBytes(ini_get('upload_max_filesize'));
      $allowedExtensions = array('jpg', 'jpeg', 'png', 'gif');
      $allowedExtensionsMime = array('jpg' => 'image/jpeg', 'png' => 'image/png', 'gif' => 'image/gif');
  ...
  static function createimageThumbnail($src, $dest, $destWidht, $destHeight ,$crop = true, $quality = 95){
    ...
    if($crop == 1){ 
      $CreateNewImage = self::oss_createImage($src, $dest, $destWidht, $destHeight ,$crop, $quality);
      return;
    }
    // Creating the Canvas
  ...
  static function oss_createImage($src, $dest, $destWidht, $destHeight ,$crop = true, $quality = 95){
    $info = @getimagesize($src, $imageinfo);
    $sWidth = $info[0];
    $sHeight = $info[1];
  ...
  static function checkJavaScriptIncludedOSS($name) {
  ...
  static function checkStylesIncludedOSS($name) {
